...
| **`#105`** | Exit the demo via the sidebar wordmark | `#105` | In a demo sandbox, the "Workflow Blueprint" sidebar wordmark now signs out the demo session and returns to the landing page (reuses `handleLogout` → `POST /api/auth/sign-out` → `/`) instead of linking to `/dashboard`. Gated on `user.isDemo` (#102); real users' wordmark still links to the dashboard. Fixes demo visitors being unable to reach the landing page (the `/`→`/dashboard` auth redirect kept bouncing them back). UI-only; app-shell tests cover both branches. |
